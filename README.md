# UMINHO_Projeto_Integrado

Repositório referente ao Projeto Integrado do 1º semestre de 2026

Grupo:

- Cinthia Ungefehr (PG58422)
- João Pedro Barbosa (PG58984)
- Lóren Narrane (PG61985)

Organização:

- 📁 analysis: Diretório contendo notebooks de exploração de dados
    - [exploration_processed.ipynb](analysis/exploration_processed.ipynb)
- 📁 processing: Diretório contendo programas para processamento dos dados
    - [processing_eda_phase1.py](processing/processing_eda_phase1.py)
    - [processing_eda_phase2.py](processing/processing_eda_phase2.py)
    - [processing_ppg_phase1.py](processing/processing_ppg_phase1.py)
    - [processing_ppg_phase2.py](processing/processing_ppg_phase2.py)
- 📁 training: Diretório contendo notebooks de treinamento de modelos
