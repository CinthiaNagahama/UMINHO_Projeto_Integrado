...
def prep_ppg(df_ppg, df_eventos):
    ...
    valid_labels = ['B1','B2','B3','B4','B5','B6','T1','T2','T3','T4','T5','T6']
...
def get_baselines(df_ppg):
    return df_ppg[df_ppg['label'].isin(['B1', 'B2', 'B3', 'B4', 'B5', 'B6'])].copy()

def get_conditions(df_ppg, df_eventos):
    lbl2code = gen_lbl2code_dict(df_eventos)

    df_conditions = df_ppg[df_ppg['label'].isin(['T1', 'T2', 'T3', 'T4', 'T5', 'T6'])].copy()
